...
public class CopyAction extends AbstractAction {
	 ...
	 * Set to false if using a from file created from other actions.
	 */
	...
	public String toHuman() {
		...
		.appendnl("<from> is file?    %s", from.isFile())
		.appendnl("<to> exists?       %s", to.exists());
		if (to.exists()) {
			desc.appendnl("<to> is directory? %s", to.isDirectory());
		}
		if (fromIsDirectoryButToIsFile(from, to)) {
			desc.appendnl("<from> is directory but <to> is file.")
			.appendnl("this action fails");
		}
		if ((to.exists()) && (to.isDirectory())) {
