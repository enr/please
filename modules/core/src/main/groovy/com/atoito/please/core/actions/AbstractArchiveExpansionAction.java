...
/**
 * Base class for archive expansion actions.
...
public abstract class AbstractArchiveExpansionAction extends AbstractAction {
	...
	private File resolveAndValidateSource() {
		...
		if (registeredOutputs.contains(file)) {
			M.debug("I'll expand '%s'. don't know if it exists, but it's registered as output from a former action...", file.getAbsolutePath());
		} else {
	...
	public String toHuman() {
		...
		.withOutputFile(destination)
		.withUsedFile(archive, "archive file", registeredOutputs).toString();
	}
