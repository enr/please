...
/**
 * Base class for archive creation actions. Just implements getArchiveType
...
public abstract class AbstractArchiveCreationAction extends AbstractAction {
    ...
    /**
     * The destination for the zip. If it is a directory, the zip will be
     * created into.
     */
    ...
    /**
     * As in Ant task: comma- or space-separated list of patterns of files that
    ...
    /**
     * As in Ant task: comma- or space-separated list of patterns of files that
     * must be excluded. No files are excluded when omitted.
     * 
     */
    ...
    public String toHuman() {
        DescriptionBuilder desc = new DescriptionBuilder()
                .forAction(getArchiveType().id)
                .humanizedAs("archive (%s) '%s' to '%s'", getArchiveType().id, source.getAbsolutePath(),
        ...
        if ((destination.exists()) && (destination.isDirectory())) {
            desc.appendnl("archive file will be created in %s and registered as output file",
                    destination.getAbsolutePath());
        } else {
